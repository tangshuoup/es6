...

{
	...
	var _x3 = new Set([1, 2, 3]);
	var _y2 = new Set([4, 3, 2]);
	//并集
	var union = new Set([].concat(_toConsumableArray(_x3), _toConsumableArray(_y2)));
	console.log(union); //set{1,2,3,4}
	//交集
	var intersect = new Set([].concat(_toConsumableArray(_x3)).filter(function (c) {
		return _y2.has(c);
	}));
	console.log(intersect); //set{2,3}
	//差集
	var difference = new Set([].concat(_toConsumableArray(_x3)).filter(function (x) {
		return !_y2.has(x);
	}));
